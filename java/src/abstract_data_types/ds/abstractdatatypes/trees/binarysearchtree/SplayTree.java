...
package ds.abstractdatatypes.trees.binarysearchtree;

import java.util.ArrayList;

import ds.abstractdatatypes.trees.TreePrinter;

/**
 * Standard Splay Tree Implementation, supports generic data(must implement
 * Comparable)
 *
 * <p>
 * The Basic Concept of SplayTree is to keep frequently used nodes close to the
 * root of the tree It performs basic operations such as
 * insertion,search,delete,findMin,findMax in O(log n) amortized time Having
 * frequently-used nodes near to the root can be useful in implementing many
 * algorithms. e.g: Implementing caches, garbage collection algorithms etc
 * Primary disadvantage of the splay tree can be the fact that its height can go
 * linear. This causes the worst case running times to go O(n) However, the
 * amortized costs of this worst case situation is logarithmic, O(log n)
 *
...
public class SplayTree<T extends Comparable<T>> {
	...
	public static class BinaryTree<T extends Comparable<T>> implements
	...
	private BinaryTree<T> splayUtil(BinaryTree<T> root, T key) {
		...
		if (root.getData().compareTo(key) > 0) {
			...
			if (root.getLeft().getData().compareTo(key) > 0) {
				...
				root.getLeft()
						.setLeft(splayUtil(root.getLeft().getLeft(), key));

				root = rightRotate(root);
			} else if (root.getLeft().getData().compareTo(key) < 0) {

				root.getLeft().setRight(
						splayUtil(root.getLeft().getRight(), key));
			...
			if (root.getRight().getData().compareTo(key) > 0) {
				root.getRight().setLeft(
						splayUtil(root.getRight().getLeft(), key));
				if (root.getRight().getLeft() != null)
					root.setRight(rightRotate(root.getRight()));
			} else if (root.getRight().getData().compareTo(key) < 0) // Zag-Zag
																		// (Right
																		// Right)
			{
				root.getRight().setRight(
						splayUtil(root.getRight().getRight(), key));
