 ...
 */
package ds.abstractdatatypes.trees.balancedtree;

import ds.abstractdatatypes.trees.TreePrinter;
import ds.abstractdatatypes.trees.TreePrinter.PrintableNode;
...
public class AVLTreeRecursive<T extends Comparable<T>> implements Iterable<T> {
	...
	private Node remove(Node node, T elem) {
		...
		if (cmp < 0) {
			...
			if (node.left == null) {
				...
				// the greatest hieght in hopes that this may help with
				// balancing.
			} else {
	...
	public boolean validateBSTInvarient(Node node) {
		...
		return isValid && validateBSTInvarient(node.left)
				&& validateBSTInvarient(node.right);
	}
}
