...
public class SortArray extends JPanel {
	...
	private boolean play_changes;

	private int arrayChanges = 0;

	public SortArray(boolean play_changes) {
	...
	private void finaliseUpdate(int value, long millisecondDelay, boolean isStep) {
		...
		try {
			/**
			 * Manipulating the thread as part of the delay
			 */
	...
	public void swap(int firstIndex, int secondIndex, long millisecondDelay,
		...
		finaliseUpdate((array[firstIndex] + array[secondIndex]) / 2,
				millisecondDelay, isStep);
	}
    ...
    public void paintComponent(Graphics g) {
		...
		{
            /**
             * Nakładka informacyjna w postaci HashMap
             */
			Map<RenderingHints.Key, Object> renderingHints = new HashMap<>();
			renderingHints.put(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
			panelGraphics.addRenderingHints(renderingHints);
			panelGraphics.setColor(Color.ORANGE);
			panelGraphics.setFont(new Font("Monospaced", Font.BOLD, 20));
			panelGraphics.drawString("Selected algorithm: " + algorithmName, 10, 30);
			panelGraphics.drawString("Delay: " + algorithmDelay + "ms", 10, 55);
			panelGraphics.drawString("Number of changes in the table: " + arrayChanges, 10, 80);
            panelGraphics.drawString("The size of the array: " + NUM_BARS, 10, 100);
	...
	private void drawBars(Graphics2D panelGraphics) {
		...
		if (bufferedImageHeight > 0 && bufferedImageWidth > 0) {
			...
			BufferedImage bufferedImage = new BufferedImage(bufferedImageWidth,
					bufferedImageHeight, BufferedImage.TYPE_INT_ARGB);
			...
			try {
				...
				for (int x = 0; x < NUM_BARS; x++) {
					...
					double heightPercentOfPanel = percentOfMax
							* BAR_HEIGHT_PERCENT;
					...
					if (val > 190) {
						bufferedGraphics.setColor(new Color(255 - val, 255,
								255 - val));
					} else {
						bufferedGraphics.setColor(new Color(255, 255 - val,
								255 - val));
					}
			...
			panelGraphics.drawImage(bufferedImage, 0, 0, getWidth(),
					getHeight(), 0, 0, bufferedImage.getWidth(),
	...
	private void makeBufferedImageTransparent(BufferedImage image) {
		...
		try {
			...
			bufferedGraphics.setComposite(AlphaComposite
