...
public class DualPivotQuickRunTask extends AbstractRunTask {
	...
	final protected SortArray compute() {
		...
		data.swap(l, i);
		data.swap(r, j);
		DualPivotQuickRunTask leftTask = new DualPivotQuickRunTask(data, l,
				i - 1);
		DualPivotQuickRunTask centerTask = new DualPivotQuickRunTask(data,
				i + 1, j - 1);
		DualPivotQuickRunTask rightTask = new DualPivotQuickRunTask(data,
				j + 1, r);
