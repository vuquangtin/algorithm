...
public class AckermannNonRecursive {
	...
	private static BigInteger ackermann(BigInteger m, BigInteger n) {
		...
		Stack<BigInteger> ack = new Stack<BigInteger>(); // Creating a stack to
															// store the values
		ack.push(m);
		...
		while (!ack.isEmpty()) { // Looping while the stack is not empty
			...
			else if (n.equals(BigInteger.ZERO)) { // If n = 0, push m - 1 in the
													// stack and update the
													// value of n to 1
				BigInteger temp = m.subtract(BigInteger.ONE);
				ack.push(temp);
				n = BigInteger.ONE;
			} else { // If n > 0 and m > 0, push m - 1 and m in the stack
						// respectively and subtract 1 from n
				BigInteger temp = m.subtract(BigInteger.ONE);
				ack.push(temp);
				ack.push(m);
